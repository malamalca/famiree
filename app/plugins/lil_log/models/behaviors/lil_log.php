 ...
 * @co-author Miha Nahtigal
 * @category Behavior
 * @version 2.1
 * @modified 27.jan 2008 by alexander
 */

class LilLogBehavior extends ModelBehavior
{
	public $user = NULL;
	...
	function setup(&$Model, $config = array()) {
		...
		$this->Log =& ClassRegistry::init('LilLogs.Log');
	...
	function afterDelete(&$Model) {
		...
		$logData = array();
		 if (isset($this->Log->_schema['description'])) {
			...
			$logData['Log']['description'] .= __d('lil_log', 'deleted',TRUE);
		 }		
    ...
    function afterSave(&$Model,$created) {
		...
		if (isset($this->Log->_schema['description'])) {		
	    	...
	    	if ($created) {
	    		$logData['Log']['description'] .= __d('lil_log', 'added',TRUE);
	    	} else {
	    		$logData['Log']['description'] .= __d('lil_log', 'updated',TRUE);
	    	}  
		}     
