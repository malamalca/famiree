	...
	class DateHelper extends Helper {
		...
		function niceShort($dateString = null, $userOffset = null, $showZeroTime=true) {
			...
			if ($this->isToday($date)) {
				$ret = __('Today', true).$time;
			} elseif ($this->wasYesterday($date)) {
				$ret = __('Yesterday', true).$time;
			} else {
		...
		function timeAgoInWords($dateTime, $options = array()) {
			...
			if ($diff > abs($now - $this->fromString($end))) {
				...
				if ($years > 0) {
                    ...
                    $relativeDate .= __('yesterday', true);
					$backwards = true;
				} elseif (abs($days) > 0) {
					// days and hours
					$relativeDate .= ($relativeDate ? ', ' : '') . $days . ' ' . __n('day', 'days', $days, true);
					if ($time) $relativeDate .= $hours > 0 ? ($relativeDate ? ', ' : '') . $hours . ' ' . __n('hour', 'hours', $hours, true) : '';
                } elseif (strftime('%d', $now)-strftime('%d', $inSeconds)==0 && !$time) {
					// days and hours
					$relativeDate .= __('today', true);
					$backwards = true;
				} elseif (abs($hours) > 0) {
					// hours and minutes
